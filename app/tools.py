...
Built-in tools NEXA can call directly, without any LLM:
  ...
  - get_datetime_info(): answers date/time/day questions.
"""
...
def safe_calculate(expression: str) -> float:
    ...
    Supports + - * / // % ** and parentheses. Raises CalculatorError on
    anything else (names, calls, attribute access, comprehensions, etc.)
    so this can never be used to execute arbitrary code, unlike eval().
    """
    try:
...
def get_datetime_info(query: str) -> str:
    """Answer simple date/time questions with plain-English text."""
    ...
    return (
        f"Right now it's {now.strftime('%A, %d %B %Y, %I:%M %p')}."
    )
